...
```text
organization_chart/
├─ docker-compose.yml         # postgres + api (+ web in prod)
├─ apps/
│  ├─ api/                    # NestJS
│  └─ web/                    # React + Vite
└─ packages/
...
TypeORM is the NestJS-idiomatic ORM (decorator entities, DI-friendly repositories, first-class migrations). **Alternative considered:** Prisma — great DX, but a second schema language and generator; TypeORM keeps the model in TypeScript next to the Nest modules. PostgreSQL over **SQLite** (the other free option) to demonstrate migrations, Docker orchestration, and a client/server DB; the trade-off (a Docker dependency) is mitigated by Compose.
...
- **Reviewer must have Docker** → *Mitigation:* single `docker compose up`, plus a documented non-Docker path (local Postgres) in the README.
...
2. Scaffold the monorepo (`apps/api`, `apps/web`, `packages/domain`) and `docker-compose.yml`.
3. Port the pure domain logic from the old `buildOrg.ts`/`config.ts` into `packages/domain` with its tests.
4. Stand up Postgres + TypeORM migrations + the seed/import; verify counts (20 departments, 4 roots, 95 employees, 3 兼務).
