...
- [x] 2.6 Port/extend unit tests; `packages/domain` passes `npm test` with no framework deps
...
- [x] 3.3 Generate the initial TypeORM migration and run it against Postgres
- [x] 3.4 Add repositories/providers and confirm the schema matches the additive-only design
...
- [x] 5.4 Add integration tests covering hierarchy, rank order, disambiguation, and `(兼)` placement
...
- [x] 11.4 Verify a new concurrent posting appears on the chart with the sourced `(兼)` chip in the target department
...
- [x] 13.11 Wire the whole suite to run in one command and headless in CI; make E2E deterministic against the seeded fixtures
